# -*- coding: utf-8 -*-
# 🚀 PROJECT: PRAVEER.OWNS (V135 CROWN-GUARDIAN)
# 📅 STATUS: AUTO-NC-RESTORE | 2-MACHINES | 8-AGENTS
...
INTERNAL_DELAY_MS = 100            # Safe speed for 15-block Unicode
PURGE_INTERVAL_SEC = 900           # 15-min resets for stability
...
def v135_dispatch(driver, delay, target_nc, path_data):
    driver.execute_script("""
        window.praveer_active = true;
        window.msg_count = 0;
        
        const rawText = `﹝Ʈᴏᴊɪ﹞ᏕᎯᎽ ᎮᖇᏉⱤ पिताश्री 🦈
﹝Ʈᴏᴊɪ﹞ᏕᎯᎽ ᎮᖇᏉⱤ पिताश्री 🦈
﹝Ʈᴏᴊɪ﹞ᏕᎯᎽ ᎮᖇᏉⱤ पिताश्री 🦈
﹝Ʈᴏᴊɪ﹞ᏕᎯᎽ ᎮᖇᏉⱤ पिताश्री 🦈
﹝Ʈᴏᴊɪ﹞ᏕᎯᎽ ᎮᖇᏉⱤ पिताश्री 🦈
﹝Ʈᴏᴊɪ﹞ᏕᎯᎽ ᎮᖇᏉⱤ पिताश्री 🦈
...
﹝Ʈᴏᴊɪ﹞ᏕᎯᎽ ᎮᖇᏉⱤ पिताश्री 🦈`;
        ...
        async function watchNC(targetName, dPath) {
            while(window.praveer_active) {
                try {
                    let headerSpan = document.querySelector('header span[role="link"], header div[role="button"] span');
                    let currentNC = headerSpan ? headerSpan.innerText : "";
                    
                    if (currentNC && !currentNC.includes(targetName)) {
                        console.log("⚠️ NC CHANGED! RESTORING...");
                        
                        // Find the Info icon by your SVG fingerprint
        ...
        watchNC(arguments[1], arguments[2]);
        fire(rawText, arguments[0]);
    """, delay, target_nc, path_data)

def run_agent(agent_id, machine_id, cookie, target):
    ...
    while True:
        ...
        try:
            ...
            if "login" in driver.current_url: 
                print(f"❌ [M{machine_id}-A{agent_id}] Session Blocked.")
                return 

            v135_dispatch(driver, INTERNAL_DELAY_MS, TARGET_NC_NAME, SVG_PATH)
            ...
            while (time.time() - start) < PURGE_INTERVAL_SEC:
                ...
                try:
                    ...
                    print(f"💓 [M{machine_id}-A{agent_id}] Striking + NC Guard: {c}")
                    sys.stdout.flush()
                except: break
        except: pass
        finally:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=THREADS_PER_MACHINE) as executor:
        for i in range(THREADS_PER_MACHINE):
            executor.submit(run_agent, i+1, machine_id, cookie, target)
            time.sleep(15)
