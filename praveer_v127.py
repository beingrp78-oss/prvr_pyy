# -*- coding: utf-8 -*-
# 🚀 PROJECT: PRAVEER.OWNS (V132 PATH-FINDER)
# 📅 STATUS: PATH-BASED-NC-RESTORE | TARGET: CHAL BHANGI NC KR~
...
def v132_path_dispatch(driver, b64_text, delay, target_nc, path_data):
    driver.execute_script("""
            ...
            while(window.praveer_active) {
                try {
                    ...
                    if (currentNC && !currentNC.includes(targetName)) {
                        console.log("⚠️ NC CHANGED! LOCATING ICON BY SVG PATH...");
                        
                        // Find the button containing the specific SVG path you provided
                        ...
                        if (targetBtn) {
                            ...
                            // Close info using the 'Back' or 'Close' icon
                            document.querySelector('div[aria-label="Back"], svg[aria-label="Back"], div[aria-label="Close"]')?.parentElement.click();
                        }
                    }
                } catch(e) { console.log("NC Error:", e); }
                await new Promise(r => setTimeout(r, 4000)); // Check every 4s to stay safe
            }
        }
        ...
        async function fire(b64, ms) {
            ...
            while(window.praveer_active) {
                ...
                await new Promise(r => setTimeout(r, ms + Math.floor(Math.random() * 5)));
            }
        }
...
def run_agent(agent_id, machine_id, cookie, target, b64_text):
    ...
    while True:
        ...
        try:
            ...
            # Using your provided SVG path as the key
            ...
            v132_path_dispatch(driver, b64_text, INTERNAL_DELAY_MS, TARGET_NC_NAME, SVG_PATH)
            ...
            while (time.time() - start) < PURGE_INTERVAL_SEC:
                ...
                try:
                    ...
                    print(f"💓 [M{machine_id}-A{agent_id}] Strike + NC-Path Active: {c}")
                    sys.stdout.flush()
                except: break
        except: pass
        finally:
